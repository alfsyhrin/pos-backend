...
const ProductModel = {
  ...
  // Search
  async search(connOrStoreId, maybeStoreId, searchTerm, limit = 5000) {
    const hasConn = connOrStoreId && typeof connOrStoreId.execute === 'function';
    const db = hasConn ? connOrStoreId : pool;
    const storeId = hasConn ? maybeStoreId : connOrStoreId;
    const term = hasConn ? searchTerm : maybeStoreId;
    const lim = hasConn ? limit : searchTerm || 5000;

    // validate limit dan masukkan langsung
    const limVal = (lim !== undefined && Number.isFinite(Number(lim)) && Number(lim) > 0) ? Math.floor(Number(lim)) : 5000;
  ...
  simpleSearch: async function (conn, storeId, q, limit = 5000) {
    let query = `SELECT * FROM products WHERE store_id = ?`;
    const params = [storeId];
    ...
    const lim = (limit !== undefined && Number.isFinite(Number(limit)) && Number(limit) > 0) ? Math.floor(Number(limit)) : 5000;
    query += ` LIMIT ${lim}`;
